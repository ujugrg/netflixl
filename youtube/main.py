...
def _auth_from_env() -> dict[str, Any] | None:
    raw = os.getenv("YTMUSIC_AUTH_JSON", "").strip()
    if not raw:
        return None
    ...
    try:
        ...
        raise APIError(500, "YTMUSIC_AUTH_JSON is not valid JSON.") from exc

    if not isinstance(parsed, dict):
        raise APIError(500, "YTMUSIC_AUTH_JSON must be a JSON object.")
...
def get_ytmusic_client() -> YTMusic:
    auth_env = _auth_from_env()
    if auth_env:
        return YTMusic(auth_env)
...
def get_ytmusic_client_required() -> YTMusic:
    ...
    if not auth_path:
        raise APIError(
            503,
            "YouTube Music authentication is not configured. Add YTMUSIC_AUTH_JSON or headers_auth.json.",
        )
    return YTMusic(str(auth_path))
